...
int main(int argc, char** argv)
{
	...
	//Debugging: Print unsorted array
	...
	std::cout << std::endl << "Sorted with parallel merge sort: " << std::endl;
	//display(array, size); //display whole array
	display(array, 5); //display first five elements
...
// Merge function used in mSort()
// It takes two sorted subarrays (defined by left, middle, and right) and merges them together into a single sorted array.
// mSort() recursively breaks the entire array down into single-item arrays, which are then merged together in larger and larger chunks.
void merge(int* array, int left, int middle, int right)
{
...
void mSort(int* array, int left, int right)
{
	...
	if (left < right)
	{
		...
		#pragma omp parallel
		{
			//		#pragma omp single // Calling thread is the taskmaster
			//		{
			#pragma omp task // Assign this to another thread
			{ mSort(array, left, middle); }
			// std::cout << omp_get_thread_num() << " ";} // Sort left half of subarray
			mSort(array, middle + 1, right); // Calling thread sorts right half of subarray
			#pragma omp taskwait // Wait for other thread to finish
			merge(array, left, middle, right); // Calling thread merges sorted halves 
	//		}
		}
	}
